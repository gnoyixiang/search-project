...
export class BubbleChartComponent implements OnInit {
  ...
  subscribeData(){
    this.subData = this.searchService.analysisSubject.subscribe(
      (data) => {
        console.log('>>> data in analysis subject subscription ' + data.length);
        this.analysis_data = data;
      },
      (error) => {
        console.log('>>> error in analysis subject subscription ' + error);
      }
    )
  }
    ...
    if(this.analysis_data.length!=0){
      ...
      node.append("circle")
          ...
          .style("fill", function(d) { 
            console.log(">>> category: " + d.data.miScore);                
            return color(d.data.miScore/10); 
          });
  ...
  classes(root) {
    // var classes = [];    
    // var packageNameList = [];
    // function recurse(name, node, list) {
    //   console.log(">>> packageNameList: " + list);
    //   if (node.children) {
    //     console.log(">>> if true");        
    ...
    //       recurse(node.name, child, list);
    //     });
    //   }
    //   else {
    //     console.log(">>> if false");                
    //     console.log(">>> packageName: " + name);
    //     console.log(">>> className: " + node.name);
    //     console.log(">>> value: " + node.size);
    //     if(!packageNameList.includes(name)){
    //       packageNameList.push(name);        
    //     }   
    //     classes.push({
    //       packageName: name, 
    //       className: node.name, 
    //       value: node.size, 
    //       category: packageNameList.indexOf(name)/packageNameList.length
    //     });  
    //   }
    // }

    // recurse(null, root, packageNameList);

    return {children: root};
  }

  addPackageNamesFromClasses(classes){
